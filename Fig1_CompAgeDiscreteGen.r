...
for (i in unique(seq$c)) {
  for (j in unique(seq$u)) {
    ...
    LambdaRog[which(unique(seq$c) ==i), which(unique(seq$u) ==j)] <- result[[which(seq$c==i & seq$u==j & seq$max_age == 2 & seq$IL_Only == "No")]]$lambda[unique(seq$tmax)]
    LambdaRogIL[which(unique(seq$c) ==i), which(unique(seq$u) ==j)] <- result[[which(seq$c==i & seq$u==j & seq$max_age == 2 & seq$IL_Only == "Yes")]]$lambda[unique(seq$tmax)]
    SLRog[which(unique(seq$c) ==i), which(unique(seq$u) ==j)] <- result[[which(seq$c==i & seq$u==j & seq$max_age == 2 & seq$IL_Only == "No")]]$SL[unique(seq$tmax)]
    
    
    LambdaAge[which(unique(seq$c) ==i), which(unique(seq$u) ==j)] <- result[[which(seq$c==i & seq$u==j & seq$max_age == 80 & seq$IL_Only == "No")]]$lambda[unique(seq$tmax)]
    LambdaAgeIL[which(unique(seq$c) ==i), which(unique(seq$u) ==j)] <- result[[which(seq$c==i & seq$u==j & seq$max_age == 80 & seq$IL_Only == "Yes")]]$lambda[unique(seq$tmax)]
    SLAge[which(unique(seq$c) ==i), which(unique(seq$u) ==j)] <- result[[which(seq$c==i & seq$u==j & seq$max_age == 80 & seq$IL_Only == "No")]]$SL[unique(seq$tmax)]
  }
}
...
mtext(side = 3, line = 1, "Rogers' Model (Discrete Generations)")
legend("bottomleft",title = expression(paste("Individual Learning cost ",italic("c"))), c("0.01","0.05","0.1"), col = c("black", "darkred", "orange"), lty=1:3,lwd=3, cex =0.85, bty = "n")
...
axis(side = 1, at = 1:6, labels =  c("0.001", "0.005", "0.01", "0.05", "0.1", "0.2"))
mtext(side = 3, line = 1, "Age-structured Model")
...
legend("bottomleft",title = "Individual Learners only", c("","",""), pch=16:18, lty =0, cex =1.1,  bty = "n", ncol = 3)

points(1:6, LambdaRogIL[1,], pch=16, cex = 1.4, col = "black")
points(1:6, LambdaRogIL[2,], pch=17, cex = 1.4, col = "black")
points(1:6, LambdaRogIL[3,], pch=18, cex = 1.4, col = "black")
...
points(1:6, LambdaAgeIL[1,], pch=16, cex = 1.4, col = "black")
points(1:6, LambdaAgeIL[2,], pch=17, cex = 1.4, col = "black")
points(1:6, LambdaAgeIL[3,], pch=18, cex = 1.4, col = "black")
